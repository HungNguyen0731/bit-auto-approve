...
5. On every Mac that needs corporate-VPN access, run the one-time portable setup below (or install a signed package when available), pair the Worker, send its token through the encrypted worker envelope, and set jobs to **Local Worker** mode.

The Coolify server does **not** inherit a developer's VPN. Do not leave approval jobs in `local` mode when Bitbucket is IP-allowlisted; that mode executes on the cloud server. The cloud container cannot open a user's Terminal by itself; the cloud **Run in Terminal** action needs the one-time Mac launcher below.

For a cloud Control Plane, **Run in Terminal** works after a one-time Mac setup. Open the Control Plane at its HTTPS domain, sign in, then use **Local Workers → Copy setup command** and review/paste it into Terminal on the Mac. It downloads the Worker bundle from that HTTPS Control Plane and a SHA-256-verified Node 24 runtime from nodejs.org; it builds the Keychain helper locally with Apple Command Line Tools (`xcode-select --install` if missing; installing those tools may prompt for administrator approval), and creates a user-owned AppleScript URL handler in `~/Applications` without a `.pkg`. The handler trusts only the Control Plane origin used during setup. Then click **Run in Terminal**, approve the browser's “Open Bitbucket PR Worker Portable” prompt, and leave Terminal open. Existing pairing and Keychain credentials are reused for subsequent runs; set jobs to **Local Worker** mode. The bootstrap route contains no credentials and does not grant pairing: the owner session/CSRF still creates a ten-minute pairing code, and the Bitbucket token is delivered through the existing encrypted Worker envelope. The local handler is ad-hoc signed (not Apple-notarized); inspect the command before running and do not disable Gatekeeper globally.

Run the setup from the Mac user's normal Terminal session, **never from a root shell or with `sudo`**. Otherwise `~/Applications` and the Worker Keychain would belong to root rather than to the browser user; the setup refuses root before downloading anything. If a root-shell attempt failed during Swift compilation, exit that shell and copy a fresh setup command from the website in the normal user's Terminal.

Public HTTP/IP access cannot bootstrap or pair a Worker in production. Confirm the HTTPS domain has a valid certificate before running the setup command; do not add `curl -k` or bypass certificate verification. The old local-only **Run in Terminal** path remains available when the Control Plane itself runs on the Mac.
